...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'upload') {
    ...
    if (empty($entries)) {
        ...
        header('Location: /admin/about-media.php?upload_error=' . urlencode($msg)); exit;
    }
    ...
    foreach ($entries as $entry) {
        ...
        if (!in_array($ext, $allowed)) { $errors[] = '.' . $ext . ' not allowed (images only)'; continue; }
        ...
        if (move_uploaded_file($entry['tmp'], $dest)) {
            $rel = '/uploads/about/' . basename($dest);
    ...
    header('Location: /admin/about-media.php'
        . ($upload_ok ? '?uploaded=1' : '?upload_error=' . urlencode(implode('; ', $errors))));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
    ...
    if ($fname !== '') {
        $path = get_about_media_path() . '/' . $fname;
...
<div class="header">
    <h1>About Page Photo</h1>
    <a href="/admin/dashboard.php" class="btn">&larr; Dashboard</a>
</div>

<div class="group">
    <div class="group-title">Sidebar Photo (<?= count($media) ?> file<?= count($media) !== 1 ? 's' : '' ?>)</div>

    <?php if (isset($_GET['uploaded'])): ?>
        <p class="upload-ok">Photo uploaded successfully.</p>
    <?php endif; ?>
    ...
    <?php if (empty($media)): ?>
        <p class="media-empty" id="about-empty">No photo uploaded yet.</p>
    <?php endif; ?>

    <div class="media-grid" id="about-grid"<?= empty($media) ? ' style="display:none"' : '' ?>>
        <?php foreach ($media as $f): ?>
        <div class="media-item">
            <img src="<?= hv($f['url']) ?>" alt="">
            <div class="media-item-name"><?= hv($f['name']) ?></div>
            <form method="POST" class="del-form" onsubmit="return confirm('Delete this photo?')">
    ...
    <p class="note">Only the first photo is shown on the about page. Upload one portrait-orientation image.</p>

    <form id="about-upload-form" enctype="multipart/form-data">
        <input type="hidden" name="action" value="upload">
        <div class="upload-row">
            <input type="file" name="media[]" accept="image/*">
            <button type="submit" class="upload-btn" id="about-upload-btn">Upload</button>
            <span id="about-upload-status" style="font-size:0.78rem"></span>
        </div>
        <p class="hint">Allowed: jpg, png, webp, gif</p>
    </form>
</div>

<script>
(function () {
    const form   = document.getElementById('about-upload-form');
    const btn    = document.getElementById('about-upload-btn');
    const status = document.getElementById('about-upload-status');
        ...
        try {
            ...
            if (data.files && data.files.length) {
                ...
                data.files.forEach(function (f) {
                    ...
                    delForm.addEventListener('submit', function(e) { if (!confirm('Delete this photo?')) e.preventDefault(); });
            ...
            if (data.errors && data.errors.length) {
                ...
                status.textContent = '✓ Uploaded'; status.style.color = '#155724';
                fileInput.value = '';
