...
$items = get_all_experiments_by_category('lab');

function lab_exp_to_embed_url(string $url): string {
    if (str_contains($url, 'player.vimeo.com') || str_contains($url, 'youtube.com/embed')) return $url;
    if (preg_match('/(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]+)/', $url, $m)) {
        return 'https://www.youtube.com/embed/' . $m[1] . '?rel=0';
    }
    if (preg_match('/vimeo\.com\/(\d+)([?&][^#]*)?/', $url, $m)) {
        return 'https://player.vimeo.com/video/' . $m[1] . ($m[2] ?? '');
    }
    return $url;
}
...
foreach ($items as $item) {
    ...
    if ($vid_url !== '') {
        $media_list[] = ['ext' => 'embed', 'url' => lab_exp_to_embed_url($vid_url)];
    }
    foreach ($media as $f) {
        $ext = $f['ext'];
        if (in_array($ext, ['jpg','jpeg','png','webp','gif','mp4','webm','mov','pdf','doc','docx'])) {
            ...
            $content = (isset($f['path']) && is_file($f['path'])) ? file_get_contents($f['path']) : '';
...
render_header(t('lab.page_title'));
?>
...
<div class="exp-wrap">
  ...
  <?php if (empty($items)): ?>
    <p style="color:#aaa;font-style:italic;font-size:0.9rem;padding:0 var(--page-gutter,2rem)">Nothing here yet.</p>
  <?php else: ?>
...
<!-- ── Modal (same as experiments) ───────────────────────────── -->
<div id="exp-modal" class="exp-modal" role="dialog" aria-modal="true" aria-label="Lab viewer">
...
<script>
const EXPERIMENTS = <?= json_encode($all_items_js, JSON_HEX_TAG | JSON_UNESCAPED_UNICODE) ?>;
let currentExp = 0, currentMedia = 0;
...
function renderModal(expIdx, mediaIdx) {
    const exp = EXPERIMENTS[expIdx], media = exp.media;
    catEl.textContent = exp.category || ''; titleEl.textContent = exp.title;
    dateEl.textContent = exp.date || ''; descEl.textContent = exp.description || '';
    mediaWrap.innerHTML = '';
    if (media.length === 0) {
        ...
        if (f.ext === 'embed') {
            const wrap = document.createElement('div'); wrap.className = 'media-pdf-wrap media-pdf-wrap--modal';
            const iframe = document.createElement('iframe');
            iframe.allow = 'autoplay; fullscreen; picture-in-picture'; iframe.setAttribute('allowfullscreen',''); iframe.title = 'Video'; iframe.src = f.url;
            wrap.appendChild(iframe); mediaWrap.appendChild(wrap);
        } else if (['mp4','webm','mov'].includes(f.ext)) {
            const v = document.createElement('video'); v.src = f.url; v.controls = true; v.className = 'exp-modal-vid'; mediaWrap.appendChild(v);
        } else if (['jpg','jpeg','png','webp','gif'].includes(f.ext)) {
            const img = document.createElement('img'); img.src = f.url; img.alt = exp.title; img.className = 'exp-modal-img'; mediaWrap.appendChild(img);
        } else if (f.ext === 'pdf') {
            const wrap = document.createElement('div'); wrap.className = 'media-pdf-wrap media-pdf-wrap--modal';
            const iframe = document.createElement('iframe'); iframe.src = f.url; iframe.title = f.name || 'Document';
            wrap.appendChild(iframe); mediaWrap.appendChild(wrap);
        } else if (f.ext === 'txt') {
            const wrap = document.createElement('div'); wrap.className = 'media-txt-wrap media-txt-wrap--modal';
            const pre = document.createElement('pre'); pre.className = 'media-txt-content'; pre.textContent = f.content || '(No content)';
            wrap.appendChild(pre); mediaWrap.appendChild(wrap);
        } else if (['doc','docx'].includes(f.ext)) {
            const card = document.createElement('div'); card.className = 'media-doc-card media-doc-card--modal';
            card.innerHTML = '<span class="media-doc-badge">' + f.ext.toUpperCase() + '</span><span class="media-doc-name">' + (f.name||'') + '</span><a class="media-doc-dl" href="' + f.url + '" download>Download</a>';
    ...
    navPrev.disabled = expIdx === 0; navNext.disabled = expIdx === EXPERIMENTS.length - 1;
    currentExp = expIdx; currentMedia = mediaIdx;
}

function openModal(expIdx) { renderModal(expIdx, 0); modal.classList.add('is-open'); document.body.style.overflow = 'hidden'; modal.focus(); }
function closeModal() { modal.classList.remove('is-open'); document.body.style.overflow = ''; const v = mediaWrap.querySelector('video'); if (v) v.pause(); }
function goExp(delta) { const next = currentExp + delta; if (next < 0 || next >= EXPERIMENTS.length) return; renderModal(next, 0); }
function goMedia(idx) { const exp = EXPERIMENTS[currentExp]; if (idx < 0 || idx >= exp.media.length) return; renderModal(currentExp, idx); }

document.querySelectorAll('.exp-slider-track').forEach(track => {
    let mouseDownX = 0, mouseDownY = 0, didDrag = false;
    track.addEventListener('mousedown', e => { mouseDownX = e.clientX; mouseDownY = e.clientY; didDrag = false; });
    track.addEventListener('mousemove', e => { if (Math.abs(e.clientX - mouseDownX) > 5 || Math.abs(e.clientY - mouseDownY) > 5) didDrag = true; });
